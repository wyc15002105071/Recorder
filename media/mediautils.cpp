...
void getCurentTime(char *str, const char *format, bool usec)
{
    struct timeval current_time;
    char temp_str[50];
    char usec_str[50];
    ...
    if(usec) {
        snprintf(temp_str + strlen(temp_str), sizeof(temp_str) - strlen(temp_str), "-%03ld", current_time.tv_usec / 1000);
    }
